...
impl Line {
    ...
    fn intersections(l1: &Line, l2: &Line) -> Vec<Point> {
        let mut intersections: Vec<Point> = Vec::new();
        for p1 in &l1.points {
            for p2 in &l2.points {
                if p1.is_equal(&p2) {
                    let p: Point = Point { x: p1.x, y: p1.y };
...
fn main() {
    ...
    let mut intersections: Vec<Point> = Line::intersections(&wires[0], &wires[1]);

    let mut min = std::i64::MAX;
    let origin = Point { x: 0, y: 0 };
    for intersection in &intersections {
        let dist = Point::manhattan(&origin, &intersection);
